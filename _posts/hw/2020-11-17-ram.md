...
- burst buffer: have wider connection at column output and cache the extra bits
 so that we cget the consecutive result without even going through cas
...
- bank + row address + column address (left over) + column address (prefetch)???
