...
1. after raid expansion, we need to first rescan the disk
...
* ~~systemd service can used to mount disk~~
  * not necessary given the correct fstab settings
...
The service file can be in the following folders:  
`/etc`, `/run`, `/lib`, `/usr/lib` `<dir>/systemd/system`
